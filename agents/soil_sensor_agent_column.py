...
class SoilAgent:
    """
    Monitors soil conditions (nutrients and moisture) in a single column,
    calculates the mean, manages its energy, and initiates the Contract Net Protocol
    for fertilization requests for the entire column.
    """
    # NOTE: Refactored constructor to only take 'col' instead of 'row' and 'col'.
    def __init__(self, field, col: int, agent_id: str, fertilizer_agents: List[str]):
        self.field = field
        self.agent_id = agent_id
        self.col = col  # The column this agent monitors
    ...
    def monitor_and_act(self):
        """
        Performs energy check, monitors the entire column, calculates mean conditions,
        and initiates the Contract Net Protocol for fertilization if needed.
        """
        # NOTE: Updated print to reflect column monitoring
        print(f"[{self.agent_id}] Starting monitoring column {self.col}. Energy: {self.energy}")
        ...
        # 2. Monitor column and calculate mean
        mean_nutrients, mean_moisture = self._get_mean_soil_conditions()
        
        print(f"[{self.agent_id}] Mean Nutrients (Col {self.col}): {mean_nutrients:.2f}, Mean Moisture (Col {self.col}): {mean_moisture:.2f}")

        # 3. Check Nutrients and initiate Contract Net Protocol
        if mean_nutrients < self.monitoring_threshold:
            print(f"[{self.agent_id}] Mean Nutrients low ({mean_nutrients:.2f}). Initiating Contract Net Protocol for fertilization for column {self.col}.")
        ...
        if mean_moisture < self.monitoring_threshold:
            print(f"[{self.agent_id}] Mean Moisture low ({mean_moisture:.2f}). Requesting irrigation for column {self.col}.")
    ...
    def _get_mean_soil_conditions(self):
        """
        Calculates the mean nutrients and moisture for the entire column (self.col).
        """
        nutrients_list = []
        moisture_list = []
        
        # Iterate over all rows in the monitored column
        for r in range(self.rows):
            # The column is self.col, which is guaranteed to be in bounds by design
            # We still check for safety, though it should not be necessary if ROWS/COLS are correct
            if 0 <= r < self.rows and 0 <= self.col < self.cols:
                # get_soil returns [temperature, nutrients, moisture]
                soil_data = self.field.get_soil(r, self.col)
                if soil_data is not None:
                    nutrients_list.append(soil_data[1])
                    moisture_list.append(soil_data[2])
            # NOTE: No need for "Skipping out-of-bounds block" as the iteration is within bounds.

        if not nutrients_list:
            return 0.0, 0.0 # Should not happen if ROWS > 0
    ...
    def _send_cfp_task(self, receiver_id: str, task_type: str, cfp_id: str):
        """Sends a CFP to a single executor agent for the entire column."""
        # Required amount is an estimate for the entire column (ROWS blocks)
        # Assuming 1 KG/block for fertilizer, 100 L/block for water
        required_amount = self.rows * 1.0 if task_type == "fertilize_application" else self.rows * 100.0
        ...
        # NOTE: The zone is the start of the column (row 0, self.col)
        zone = [0, self.col]
        ...
        print(f"[{self.agent_id}] CFP to {receiver_id} (Col {self.col}): \n{json.dumps(cfp_message, indent=4)}")
